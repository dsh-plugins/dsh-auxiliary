...
同一折叠区内还提供**思考强度**配置：列表按行添加/删除档位（仅接受 `off`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`），铅笔按钮可批量输入 `[low, high, max]` 形式的文本（含非法档位时整段拒绝），默认思考强度下拉只列出当前列表中的档位；新增模型的列表与默认项均为空白。这些字段作为插件自有原始字段（`thinkingLevels` / `defaultThinkingLevel`）保存，改动同样等提供方 Apply 后写入，避免 revision 冲突。
