...
复选框注入到每个用户自有的 `llm-pi-ai` 模型行，并放在与 **上下文窗口**、**最大输出 token** 相同的行内容量折叠里，折叠收起时不显示、展开后出现。自定义提供方创建卡中**正在添加**的模型行同样会获得复选框；刚添加但尚未填写模型 ID 的行在展开该折叠后会显示禁用的复选框，填入 ID 后立即启用。修改已有模型的 ID 时，已保存的能力标记会随 ID 迁移到新 ID。勾选的标记先在浏览器中记录，待页面保存提供方设置（点 Apply）后写入该模型，因此可以一边修改其它模型字段一边设置图片能力，不会与页面自身的保存产生“设置已被其他地方改动”的 revision 冲突。无法携带标记的行会显示说明而非静默缺失：DeepSeek 官方（或其他非 pi-ai 适配器）行提示标记仅限 `llm-pi-ai`；尚未保存到用户区的 pi-ai 目录行提示先保存模型。

同一折叠区内还提供**思考强度**配置：列表按行添加/删除档位（仅接受 `off`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`），铅笔按钮可批量输入 `[low, high, max]` 形式的文本（含非法档位时整段拒绝），默认思考强度下拉只列出当前列表中的档位；新增模型的列表与默认项均为空白。这些配置直接读写官方的模型 `reasoningEfforts` 字段（档位 → 同名 wire 值，`off` → `null`）与提供方级 `reasoning` 默认档位——正是 pi-ai 用来决定模型是否具备推理能力、并在输入框提供推理等级选择器的同一条数据，因此该编辑器就是官方机制的作者界面，无需再维护插件私有字段。改动同样等提供方 Apply 后写入，避免 revision 冲突。

> 此前版本把档位保存在插件自有字段（`thinkingLevels` / `defaultThinkingLevel`）；升级后首次保存会自动迁移到官方字段并清除旧字段。
